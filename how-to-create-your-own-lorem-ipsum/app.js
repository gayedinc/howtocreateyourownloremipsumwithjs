...
form.addEventListener("submit", function (e) { // Bu kod, form üzerinde "submit" olayı için bir olay dinleyici ekler. 

    e.preventDefault(); // Varsayılan form gönderim davranışını engeller ve form gönderimini programatik olarak işlemenizi sağlar.

    const value = parseInt(amount.value); // Bu satır ve alttaki satır, kullanıcının girdiği sayıyı (istenen paragraf sayısı) çıkarır ve rastgele bir Lorem Ipsum metni seçmek için rastgele bir dizin oluşturur.
    const random = Math.floor(Math.random() * text.length);

    if (isNaN(value) || value <= 0 || value > 9) {
        // Giriş geçerli bir sayı değilse veya aralık dışındaysa rastgele bir paragraf gösterilir.
        result.innerHTML = `<p class="result">${text[random]}<p/>`
    }
    else {
        // İstenen sayıda paragraf gösterilir.
